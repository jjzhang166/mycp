...
*/
#ifndef _MD5_HEAD_INCLUDED__
#define _MD5_HEAD_INCLUDED__
...
#define MD5_RES_LEN			16	//摘要的结果的字节长度。
#define MD5_RES_STR_LEN		32	//摘要的结果转成ansi字符串后的字节长度：其结果是16个字节，每个字节用两个字符来表示16进制值。
...
public:
	...
	char *            hex_digest (){  // digest as a 33-byte ascii-hex string
		...
		for (i=0; i<MD5_RES_LEN; i++)
			sprintf((char*)m_cHexResult+i*2, "%02x", digest[i]);
		//	sprintf_s<2>((char*)m_cHexResult+i*2, "%02x", digest[i]);
...
private:
	...
	void init             (){               // called by all constructors
		...
		::memset(&m_cHexResult, 0, sizeof(m_cHexResult));
		::memset(&m_cRawResult, 0, sizeof(m_cRawResult));
	}
...
#define S44 21
	...
	static void memcpy (uint1 *output, uint1 *input, uint4 len){
		unsigned int i;
		for (i = 0; i < len; i++)
			output[i] = input[i];
	}
	...
	static void memset (uint1 *output, uint1 value, uint4 len){
		unsigned int i;
		for (i = 0; i < len; i++)
			output[i] = value;
	}
...
private:
	char m_cHexResult[33];
	unsigned char m_cRawResult[16];
};
