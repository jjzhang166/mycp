...
// ****ECB模式
int aes_ecb_encrypt_full(const unsigned char *userKey,int klen,const unsigned char* in,int ilen,unsigned char* out)
{
	...
	if (ret!=0)
		return ret;
	for (int i=0;i<ilen; i+=klen)
		AES_ecb_encrypt(in+i,out+i,&key,AES_ENCRYPT);
	return 0;
}
int aes_ecb_encrypt(const unsigned char *userKey,int klen,const unsigned char* in,unsigned char* out)
{
	// 如果密钥数字是16个字节，16*8=128；则此参数值应为128；
	// 返回值： 0 成功， -1 userkey，key为空， -2： 密钥长度不是128(16*8)，192(24*8)，256(32*8)；
	int ret = 0;
...
int aes_ecb_decrypt_full(const unsigned char *userKey,int klen,const unsigned char* in,int ilen,unsigned char* out)
{
	// 如果密钥数字是16个字节，16*8=128；则此参数值应为128；
	// 返回值： 0 成功， -1 userkey，key为空， -2： 密钥长度不是128(16*8)，192(24*8)，256(32*8)；
	int ret = 0;
...
int aes_ecb_decrypt(const unsigned char *userKey,int klen,const unsigned char* in,unsigned char* out)
{
	// 如果密钥数字是16个字节，16*8=128；则此参数值应为128；
	// 返回值： 0 成功， -1 userkey，key为空， -2： 密钥长度不是128(16*8)，192(24*8)，256(32*8)；
	int ret = 0;
...
// ****CBC模式简介
int aes_cbc_encrypt(const unsigned char *userKey,int klen,const unsigned char* in,int length,unsigned char* out)
{
	// 如果密钥数字是16个字节，16*8=128；则此参数值应为128；
	// 返回值： 0 成功， -1 userkey，key为空， -2： 密钥长度不是128(16*8)，192(24*8)，256(32*8)；
	int ret = 0;
...
int aes_cbc_decrypt(const unsigned char *userKey,int klen,const unsigned char* in,int length,unsigned char* out)
{
	// 如果密钥数字是16个字节，16*8=128；则此参数值应为128；
	// 返回值： 0 成功， -1 userkey，key为空， -2： 密钥长度不是128(16*8)，192(24*8)，256(32*8)；
	int ret = 0;
