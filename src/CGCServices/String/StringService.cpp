...
protected:
	virtual bool callService(const tstring& function, const cgcValueInfo::pointer& inParam, cgcValueInfo::pointer outParam)
	{
		...
		}else if (function == "des_enc")
		{
			...
			unsigned char * lpszBuffer = new unsigned char[sInString.size()+8];
			memset(lpszBuffer,0,sInString.size()+8);
			DesEncrypt(sKey.c_str(),(unsigned char*)sInString.c_str(),lpszBuffer,sInString.size());

			const std::string sOutString((const char*)lpszBuffer);
			delete[] lpszBuffer;
		...
		}else if (function == "des_dec")
		{
			...
			unsigned char * lpszBuffer = new unsigned char[sInString.size()+8];
			memset(lpszBuffer,0,sInString.size()+8);
			DesDecrypt(sKey.c_str(),(unsigned char*)sInString.c_str(),lpszBuffer,sInString.size());
